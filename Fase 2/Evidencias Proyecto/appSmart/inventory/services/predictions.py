...
class LogisticRegressor:
    ...
    def fit(self, features: Sequence[Sequence[float]], labels: Sequence[int]) -> None:
        ...
        for _ in range(self.epochs):
            for i, row in enumerate(scaled_features):
                ...
                # Actualizar bias
                self.weights[0] += (
                    self.learning_rate * error * prediction * (1 - prediction)
                )
    ...
    def predict_single(self, row: Sequence[float]) -> float:
        ...
        scaled_row = (
            self._scale_single(row) if hasattr(self, "feature_min") else list(row)
        )
...
def generar_predicciones() -> PredictionPayload:
    ...
    if not productos:
        return PredictionPayload(
            items=[], sugerencias=0, sobrestock=0, fecha_generacion=timezone.now()
        )
    ...
    for producto in productos:
        try:
            ...
            promedio_mensual = (
                statistics.mean([r.quantity for r in historial]) if historial else 0
            )
...
def _generar_historial_sintetico(producto: Productos) -> List[PurchaseRecord]:
    ...
    for i in range(12):  # 12 meses de historial
        ...
        historial.append(
            PurchaseRecord(
                period_label=f"Mes {i+1}", quantity=quantity, revenue=revenue
            )
        )
...
def _predecir_probabilidad_compra(
    producto: Productos, historial: List[PurchaseRecord]
) -> float:
...
def _determinar_accion(
    producto: Productos, probabilidad: float, promedio: float, volatilidad: float
):
...
def exportar_reporte_compras_csv():
    ...
    output = io.StringIO()
    output.write(
        "ID,Producto,Marca,Cantidad Sugerida,Precio Unitario,Total Estimado,Motivo\n"
    )

    for item in compras:
        total = item.cantidad_sugerida * item.precio_referencia
        output.write(
            f"{item.producto.id_producto},{item.producto.title},"
