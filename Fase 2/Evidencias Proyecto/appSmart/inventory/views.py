...
def dashboard(request):
    ...
    # Top categorías
    categorias_top = (
        Productos.objects.filter(categoria1__isnull=False)
        .values("categoria1")
        .annotate(count=Count("categoria1"))
        .order_by("-count")[:5]
    )

    # Top marcas
    marcas_top = (
        Productos.objects.filter(brand__isnull=False)
        .values("brand")
    ...
    try:
        ventas_recientes = Ventas.objects.select_related("id_producto").order_by(
            "-fecha"
        )[:10]
        total_ventas = Ventas.objects.aggregate(
            total_cantidad=Sum("cantidad_vendida"), total_ingresos=Sum("total_venta")
        )
    except:
        ventas_recientes = []
    ...
    # Productos recientes
    productos_recientes = (
        Productos.objects.order_by("-datetime")[:5]
        if hasattr(Productos, "datetime")
        else Productos.objects.all()[:5]
    )

    # Mostrar alertas automáticas
    if productos_sin_stock > 10:
        messages.warning(
            request, f"⚠️ Atención: {productos_sin_stock} productos sin stock"
        )
...
def prediccion_productos(request):
    ...
    try:
        ...
        prediction_summary = {
            ...
            "promedio_probabilidad": sum(p.probabilidad for p in productos_prediccion)
            / len(productos_prediccion)
        ...
        if accion_filter and accion_filter in ["comprar", "mantener", "evaluar"]:
            productos_prediccion = [
                p for p in productos_prediccion if p.accion.lower() == accion_filter
            ]
...
def exportar_reporte_compras(request):
    ...
    try:
        ...
        productos_compra = [
            p for p in predicciones_data.items if p.accion.lower() == "comprar"
        ]
