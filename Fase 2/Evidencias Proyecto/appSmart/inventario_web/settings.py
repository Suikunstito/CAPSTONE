...
def _configure_database():
    ...
    use_sqlite_fallback = os.getenv("USE_SQLITE_FALLBACK", "True").lower() in (
        "1",
        "true",
        "yes",
        "on",
    )
    force_sqlite = os.getenv("FORCE_SQLITE", "False").lower() in (
        "1",
        "true",
        "yes",
        "on",
        ...
        if DEBUG and use_sqlite_fallback:
            try:
                ...
                if not available_drivers:
                    print(
                        "[SmartERP] ⚠️  No se encontraron drivers ODBC para SQL Server"
                    )
                ...
                print(
                    "[SmartERP] ℹ️  Si la conexión falla al iniciar, usa: FORCE_SQLITE=True"
                )
...
def _get_sqlite_config():
    ...
    print(
        "[SmartERP] 💡 Para usar SQL Server, verifica la configuración en .env.development"
    )
    return {
